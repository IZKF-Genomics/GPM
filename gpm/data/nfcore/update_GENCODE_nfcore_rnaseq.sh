...
echo "FASTA genome:      $FASTA_Genome"
echo "FASTA transcripts: $FASTA_transcripts"
echo "GTF annotation: $GTF"
echo "Target Folder:  $FOLDER"
echo "Genome name:    $NAME"
...
STAR --runThreadN ${Num_cores} --runMode genomeGenerate --genomeDir $STAR_INDEX --genomeFastaFiles $FASTA_Genome

# Run Salmon index
salmon_index="${FOLDER}/salmon_index"
salmon index -t ${FASTA_transcripts} -i $salmon_index --gencode -p ${Num_cores}
...
# Transcripts
gffread -w ${FOLDER}/${NAME}_ERCC_transcripts.gtf -g ${FOLDER}/${NAME}_ERCC.fasta ${FOLDER}/${NAME}_ERCC.gtf
# Run STAR index
STAR --runThreadN ${Num_cores} --runMode genomeGenerate --genomeDir ${STAR_INDEX}_ERCC --genomeFastaFiles ${FOLDER}/${NAME}_ERCC.fasta
# Run Salmon index
salmon index -t ${FOLDER}/${NAME}_ERCC_transcripts.fasta -i ${salmon_index}_ERCC --gencode -p ${Num_cores}

# Export the config parameters for nextflow.config
echo "Please add the following codes into nextflow.config:"
echo "'${NAME}' {
      fasta  = '${FASTA_Genome}'
      gtf = '${GTF}'
