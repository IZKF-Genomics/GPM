---
title: "Gene Ontology Analysis for TITLEDESCRIPTION"
author:
...
For gene ontology analysis, R library gprofiler2[@kolberg2020gprofiler2] was used. Using gost function, correction_method = â€œfdrâ€ and significant=TRUE. To minimize the enrichment of gene ontologies based on a small set of genes, we used three conditions for significance assessment: false discovery rate (FDR) < `r pvalueCutoff`. We used Gene Ontology (GO or by branch GO\:MF, GO\:BP, GO\:CC), Kyoto Encyclopedia of Genes and Genomes (KEGG), WikiPathways (WP) as sources.
...
run_GO <- function(label) {
  ...
  df <- df[df$padj < pvalueCutoff,]
  # de_gene_list <- gsub("\\.[0-9]*$", "", df$gene_id[(df$padj < pvalueCutoff)])
  up_gene_list <- gsub("\\.[0-9]*$", "", df$gene_id[(df$padj < pvalueCutoff)&(df$log2FoldChange>0)])
  dn_gene_list <- gsub("\\.[0-9]*$", "", df$gene_id[(df$padj < pvalueCutoff)&(df$log2FoldChange<0)])
  
  gostres <- gost(query = list("Down" = dn_gene_list,
                               "Up" = up_gene_list), 
                  multi_query = FALSE, evcodes = TRUE, 
                  correction_method = "fdr", user_threshold=pvalueCutoff,
...
## DNP20min_vs_Control_in_WT

```{r, echo=FALSE, results='markup', warning=FALSE, message=FALSE}
gostres <- run_GO(label="DNP20min_vs_Control_in_WT")
gostplot(gostres, capped = FALSE, interactive = TRUE)
```
...
## KO_vs_WT_under_Control

```{r, echo=FALSE, results='markup', warning=FALSE, message=FALSE}
gostres <- run_GO(label="KO_vs_WT_under_Control")
gostplot(gostres, capped = FALSE, interactive = TRUE)
```
