---
title: "220211 Kalmer Koschmieder MedIV scRNAseq"
author:
  - "Lin Gan, Genomics Core Facility, lgan@ukaachen.de"
date: "Last compiled on `r format(Sys.time(), '%d %B, %Y')`"
output:
  html_document:
    df_print: paged
    toc: true
    toc_float:
      toc_collapsed: true
    toc_depth: 3
    number_sections: false
    theme: lumen
bibliography: references.bib
---

```{r conig, echo=FALSE, results="hide", warning=FALSE, message=FALSE}
library("heatmaply")
library("readr")
require("tidyr")
options(warn = -1)

DIR_base <- "/data/projects/220211_Kalmer_Koschmieder_MedIV_scRNAseq/"
DIR_project <- paste0(DIR_base, "analysis/")
DIR_data <- "/data/fastq/220211_NB501289_0587_AHK5WLAFX3/"
DIR_count <- paste0(DIR_base,"count/")
DIR_qc <- paste0(DIR_base,"QC/")


```


![](https://www.medizin.rwth-aachen.de/global/show_document.asp?id=aaaaaaaaaawfyoe "IZKF")

This document is the summary of the bioinformatic analysis done by Genomic Core Facility in IZKF, RWTH Aachen Uniklinik. Please contact us for any further discussion or collaboration. If these results are used in any poster or publications, please acknowledge our core facility with the text below:

> This work was supported by the Genomics Facility, a core facility of the Interdisciplinary Center for Clinical Research (IZKF) Aachen within the Faculty of Medicine at RWTH Aachen University.

Please refer to our [Publication Policy](https://genomics.rwth-aachen.de/services/publication-policy/).


***

In this report, you are able to:

* Browse and download the files and reports in an intuitive folder structure.
* Read the QC reports and cellranger count summary reports online.
* Get the summary of the method description and citation.

After deliverying this report, we can schedule a short meeting to discuss the result, otherwise, we will close this project with this basic analysis. If you have further request and questions, please don't hestitate to contact us.

***

# Download/Browse the files

Please click the hyperlinked list below:

1. [Raw Data](https://genomics.rwth-aachen.de/data/220211_Kalmer_Koschmieder_MedIV_scRNAseq/1_Raw_data):
    * BCL files from the sequencer
    * FASTQ files after demultiplexing
2. [Processed Data](https://genomics.rwth-aachen.de/data/220211_Kalmer_Koschmieder_MedIV_scRNAseq/2_Processed_data):
    * BAM and bai files after alignment
    * raw feature bc count matrix 
    * filtered feature bc count matrix
    * cloupe file for browsing in Loupe browser
3. [Reports](https://genomics.rwth-aachen.de/data/220211_Kalmer_Koschmieder_MedIV_scRNAseq/3_Reports):
    * [MultiQC reports](https://genomics.rwth-aachen.de/data/220211_Kalmer_Koschmieder_MedIV_scRNAseq/3_Reports/multiqc/multiqc_report.html) for the quality check
    * Basic analysis of the Genomics Facility (This report)
4. [Compressed files for download](https://genomics.rwth-aachen.de/data/220211_Kalmer_Koschmieder_MedIV_scRNAseq/compressed_tars):
    * Three tar files are available for download for the above three folders.

In the command line environment, you can also download everything by the commands below with the given username and password from us:

```
wget -r --user USERNAME --password PASSWORD -A '*.tar' https://genomics.rwth-aachen.de/data/220211_Kalmer_Koschmieder_MedIV_scRNAseq/compressed_tars
```

Please make sure that you store these data in a lab-wise level. Genomics Facility won't store your data forever and these data should still be accessible even after someone leaves your lab. Please refer to our [Data Management Policy](https://genomics.rwth-aachen.de/services/data-management-policy/).

# Method description

FASTQ files were generated using cellranger mkfastq (10x Genomics). The generated fastq files are analyzed in the cellranger count pipe. This report was generated using custom scripts in R version 4.1.1. 


# summaries of the basic analysis result

```{r load, echo=FALSE, results="hide", warning=FALSE, message=FALSE}

samples <- list.dirs(path = DIR_count, full.names = F, recursive = F)

summaryFiles <- vector(mode="list", length=length(samples))
names(summaryFiles) <- samples

for (i in 1:length(samples)) {
  summaryFiles[[i]] <- paste0(DIR_count,samples[[i]],"/outs/web_summary.html")
}

```

```{r, results="asis"}

library(htmltools)


for( i in 1:length(samples)){
  
  cat("\n") 
  cat("##", "[",samples[[i]],"]","(",summaryFiles[[i]],")\n") # Create links to the summary reports
  
  cat("\n")
  
}

```

##[samples[[1]]](summaryFiles[[1]])

# References

<div id="refs"></div>


# R session information

```{r info, echo=FALSE, results='markup', warning=FALSE, message=TRUE}
sessionInfo()
```
